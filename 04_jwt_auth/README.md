...
| 方法 | 用途 | 例 |
|------|------|-----|
| **環境変数** | 最も一般的 | `export JWT_SECRET="..."` |
| **シークレット管理サービス** | 本番環境 | AWS Secrets Manager, HashiCorp Vault |
| **設定ファイル** | 開発環境のみ | `.env`ファイル（.gitignore必須） |
| **KMS** | 大規模システム | AWS KMS, GCP Cloud KMS |

```bash
# 環境変数での設定例
export JWT_SECRET_KEY="super-secret-key-here"
```
...
**絶対にやってはいけないこと:**
- ソースコードにハードコード（GitHubに公開される）
- バージョン管理に含める

### Q: 全ユーザーで同じ秘密鍵を使う？

**はい、通常は1つの秘密鍵で全ユーザーのJWTに署名する**

```
ユーザーA ログイン → JWT生成（秘密鍵で署名）→ eyJhbGci...A
ユーザーB ログイン → JWT生成（秘密鍵で署名）→ eyJhbGci...B
ユーザーC ログイン → JWT生成（秘密鍵で署名）→ eyJhbGci...C
                         ↑
                    同じ秘密鍵を使用
```
...
| 心配 | 回答 |
|------|------|
| 同じ鍵でも大丈夫？ | Payloadが違うので署名も全て異なる |
| AのJWTでBになりすませる？ | Payloadを改ざんすると署名が合わなくなる |
| 秘密鍵が漏れたら？ | 全ユーザーのJWTが偽造可能 → だから厳重に管理 |
...
1. ログイン時
   パスワード → bcryptでハッシュ比較 → 認証成功

2. JWT発行
   ユーザー情報（IDなど）→ 秘密鍵で署名 → JWT

3. 以降のリクエスト
   JWT → 秘密鍵で署名検証 → ユーザー特定
```

- **パスワード**: ログイン時の本人確認（bcryptでハッシュ化してDB保存）
- **JWT**: ログイン後の認証状態維持（秘密鍵で署名）

...
### Q: セッションIDの保存方法として一般的なものは？

#### サーバー側の保存先

| 方法 | 特徴 | 使用例 |
|------|------|--------|
| **Redis** | 最も一般的、高速、TTL機能あり | 本番環境の標準 |
| **Memcached** | Redisに似ている、シンプル | レガシーシステム |
| **DB（PostgreSQL等）** | 永続化可能、やや遅い | 小規模サービス |
| **メモリ（今回）** | 再起動で消える、共有不可 | 開発・学習用 |

#### クライアント側の保存先

| 方法 | 特徴 |
|------|------|
| **Cookie** | 最も一般的、自動送信される |
...
|------|------|
| 高速 | メモリベースなのでDBより速い |
| TTL | 有効期限を自動管理（期限切れで自動削除） |
| 共有可能 | 複数サーバーからアクセス可能 |

```
# Redisでのセッション保存イメージ
SET session:abc123 '{"username":"taro"}' EX 3600
                                         ↑ 1時間後に自動削除
```
